                        ...
                        <div class="media mb-4 align-items-center">
                            <div class="media-body">
                                ...
                                <h4 class="fs-14 font-w600 mb-1">
                                    {{ $pasien->tmp_lahir . ', ' . $pasien->tgl_lahir }}
                                </h4>
                                ...
                                <h4 class="fs-14 font-w600 mb-1">
                                    {{ $pasien->jk . ', ' . $pasien->status_menikah }}
                                </h4>

                                <span class="fs-14">{{ $pasien->alamat_lengkap }}</span>
                                <br>
                                <span class="fs-14">
                                    {{ $pasien->keluhan . ', ' . $pasien->kecamatan . ', ' . $pasien->kabupaten . ', ' . $pasien->kewarganegaraan }}
                                </span>

                                <br>

                                @if ($pasien->isRekamGigi() && auth()->user()->role_display() == "Dokter")
                                    <a href="{{ Route('rekam.gigi.odontogram', $pasien->id) }}"
                                       style="width: 120px"
                                       class="btn-rounded btn-info btn-xs mt-2">
                                        <i class="fa fa-eye"></i> Odontogram
                                    </a>
                                @endif
                            </div>
                        </div>
            ...
            <div class="card-body">
                <div class="table-responsive card-table">
                    <div class="form-group col-lg-6" style="float: right">
                        <form method="get" action="{{ url()->current() }}">
                            <div class="row">
                                <div class="col-md-12">
                                    <input type="text"
                                           class="form-control gp-search"
                                           name="keyword"
                                           value="{{ request('keyword') }}"
                                           placeholder="Cari tanggal periksa"
                                           autocomplete="off">

                                    {{-- Filter poli di-hide karena sistem fokus pada rekam medis klinik gigi --}}
                            ...
                            @foreach ($rekams as $key => $row)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            ...
                                            @if ($row->pemeriksaan_file != null)
                                                <br>
                                                <a target="__BLANK" href="{{ $row->getFilePemeriksaan() }}">
                                        ...
                                        @else
                                            <table>
                                                @foreach ($row->diagnosa() as $item)
                                                    <tr>
                                                        <td>{{ optional($item->diagnosis)->code ?? '-' }}</td>
                                                        <td>
                                                            @if ($row->status <= 2)
                                    ...
                                    <td>
                                        @if ($row->poli == "Poli Gigi")
                                            @foreach ($row->gigi() as $item)
                                                <li>{{ optional($item->tindak)->nama ?? '-' }}</li>
                                            @endforeach
                                        @else
                                            {!! $row->tindakan !!}
                                            ...
                                            @if ($row->tindakan_file != null)
                                                <br>
                                                <a target="__BLANK" href="{{ $row->getFileTindakan() }}">
                                                    <u style="color:rgb(28, 85, 231);">Lihat Foto</u>
                                                </a>
                                            @endif
                                        @endif
                                    ...
                                    <td>
                                        @if ($row->status != 5 && $row->status != 4)
                                            <div class="btn-group-vertical" role="group" aria-label="Vertical button group">
                                                @if ($row->poli != "Poli Gigi")
                                                    @if (auth()->user()->role_display() == "Dokter")
                                                        <a href="javascript:void(0)"
                                                           data-toggle="modal"
                                                           data-target="#addPemeriksaan"
                                                           data-id="{{ $row->id }}"
                                                           data-tanggal="{{ $row->tgl_rekam }}"
                                                           data-pemeriksaan="{{ $row->pemeriksaan }}"
                                                           style="width: 120px"
                                                           class="btn-rounded btn-info btn-xs addPemeriksaan">
                                                            <i class="fa fa-pencil"></i> Object
                                                        </a>
                                                    @endif
                                                ...
                                                @else
                                                    @if (auth()->user()->role_display() == "Dokter")
                                                        <a href="{{ Route('rekam.gigi.add', $row->id) }}"
                                                           style="width: 120px"
                                                           class="btn-rounded btn-info btn-xs">
                                                            <i class="fa fa-pencil"></i> Rekam
                                                        </a>
                                                        ...
                                                        @if ($row->gigi()->count() > 0)
                                                            <a href="javascript:void(0)"
                                                               ...
                                                               class="btn-rounded btn-success btn-xs addResep">
                                                                <i class="fa fa-pencil"></i> Resep Obat
                                                            </a>
                                                        @endif
                                                    @endif
                                                @endif
                                        ...
                                        @else
                                            <div class="d-flex">
                                                <a href="{{ Route('obat.pengeluaran', $row->id) }}"
                                                   style="width: 120px"
                                                   class="btn-rounded btn-primary btn-xs">
                    ...
                    <div class="dataTables_info" id="example_info" role="status" aria-live="polite">
                        Showing {{ $rekams->firstItem() }} to {{ $rekams->perPage() * $rekams->currentPage() }} of {{ $rekams->total() }} entries
                    </div>
