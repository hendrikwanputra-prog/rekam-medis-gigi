...
<div class="mr-auto">
    <ol class="breadcrumb">
        <li class="breadcrumb-item">
            <a href="{{ Route('rekam.detail', $rekam->pasien_id) }}">Rekam Medis</a>
        </li>
        <li class="breadcrumb-item active">
            <a href="#">Tambah Rekam Gigi {{ $rekam->pasien->nama }}</a>
        </li>
    </ol>
</div>
        ...
        <div class="card">
            <div class="card-body">
                <div class="basic-form">
                    ...
                    <a href="{{ Route('rekam.gigi.odontogram', $rekam->pasien_id) }}"
                       style="width: 170px"
                                ...
                                <div class="row">
                                    ...
                                    @if ($rekam->status <= 2)
                                        <div class="col-md-4">
                                            ...
                                            <div class="form-group">
                                                <label class="text-black font-w500">Catatan Dokter</label>
                                                <textarea id="catatan"
                                                          class="form-control"
                                                          rows="3"
                                                          placeholder="Masukkan catatan tambahan kondisi gigi pasien"></textarea>
                                            </div>

                                            <div class="form-group">
                                                <button type="button" onclick="addRekam()" class="btn btn-info">
                                                    + Tambah
                                                </button>
                                            </div>
                                                ...
                                                <tbody>
                                                    @foreach ($pem_gigi as $row)
                                                        <tr>
                                                            ...
                                                            <td>
                                                                @if ($rekam->status <= 2)
                                                                    <a href="{{ Route('rekam.gigi.delete', $row->id) }}"
                                                                       class="btn btn-danger shadow btn-xs sharp"
                                                                       onclick="return confirm('Yakin ingin menghapus data gigi {{ $row->elemen_gigi }}?')">
...
<script>
    function addRekam() {
        ...
        if (kondisi_gigi == "" || tindakan == "" || diagnosa == "") {
            ...
            var markup = '<tr>' +
                ...
                '<td style="width: 80px">' +
                    '<button type="button" class="btn btn-danger shadow btn-xs sharp btnDelete">' +
                        '<i class="fa fa-trash"></i>' +
                    '</button>' +
                '</td>' +
            '</tr>';
    ...
    // Hapus baris rincian yang baru ditambahkan sebelum disimpan
    ...
    jQuery(function() {
        function drawDiente(svg, parentGroup, diente) {
            ...
            var caraSuperior = svg.polygon(dienteGroup, [[0,0],[20,0],[15,5],[5,5]], sPoloygon);
            caraSuperior = $(caraSuperior).data('cara', 'S');

            var caraInferior = svg.polygon(dienteGroup, [[5,15],[15,15],[20,20],[0,20]], iPoloygon);
            caraInferior = $(caraInferior).data('cara', 'I');

            var caraDerecha = svg.polygon(dienteGroup, [[15,5],[20,0],[20,20],[15,15]], dPoloygon);
            caraDerecha = $(caraDerecha).data('cara', 'D');

            var caraIzquierda = svg.polygon(dienteGroup, [[0,0],[5,5],[5,15],[0,20]], zPoloygon);
            caraIzquierda = $(caraIzquierda).data('cara', 'Z');

            var caraCentral = svg.polygon(dienteGroup, [[5,5],[15,5],[15,15],[5,15]], cPoloygon);
            ...
            $.each([caraCentral, caraIzquierda, caraDerecha, caraInferior, caraSuperior, caraCompleto], function(index, value) {
                value.click(function() {
                    ...
                    var nomorGigi = diente.id;

                    if (!isNaN(nomorGigi)) {
                        $('#element_gigi').val(nomorGigi).trigger('change');
                    }
